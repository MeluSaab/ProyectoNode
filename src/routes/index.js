...
router.get('/artistas/:id', async (req,res) => {
    ...
    const datos = (await db.collection('artistas').doc(id).get()).data()
    res.send(datos)
    //const canciones = await datos.canciones 
})
...
router.get('/canciones/:id', async (req, res) => {
    const id = req.params.id
    const datos = (await db.collection('canciones').doc(id).get()).data()
    res.send(datos)
})
